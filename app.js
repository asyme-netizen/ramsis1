...
function renderHero() {
  ...
  document.getElementById("heroArticle").innerHTML = `
    ...
    <div class="hero-meta">${content.categoryLabel} • ${hero.readTime} دقائق • ${hero.date}</div>
...
function renderLatest(filtered = articleData) {
  ...
  grid.innerHTML = filtered.map((a) => `
    <article class="story-card glass-panel reveal visible">
      ...
      <span class="story-tag">${a.ar.categoryLabel}</span>
      <h3>${a.ar.title}</h3>
      <p>${a.ar.excerpt}</p>
...
function renderSections() {
  ...
  document.getElementById("newsSections").innerHTML = categories.map((cat) => {
    const items = articleData.filter((a) => a.category === cat.key).slice(0, 2);
    return `
...
function bindSearch(input) {
  ...
  input.addEventListener("input", (e) => {
    ...
    const filtered = articleData.filter((a) => {
      const hay = [a.ar.title, a.ar.excerpt, a.ar.categoryLabel, a.author].join(" ").toLowerCase();
...
function injectSiteContent() {
  ...
  document.getElementById("footerText").textContent = "رمسيس — منصة عربية حديثة للتاريخ والسياسة والتكنولوجيا";
}

...
function setAdminOverlay(visible) {
...
function initAdminLoginUi() {
  ...
  const closeBtn = document.getElementById("closeLoginOverlay");
  const logoutBtn = document.getElementById("logoutAdmin");
  if (!overlay || !form) return;

  const isLogged = localStorage.getItem("ramsesAdminLogged") === "1";
  setAdminOverlay(!isLogged);

  form.addEventListener("submit", (event) => {
    event.preventDefault();
    localStorage.setItem("ramsesAdminLogged", "1");
    setAdminOverlay(false);
  });

  closeBtn?.addEventListener("click", () => setAdminOverlay(false));
  overlay.querySelector(".admin-login-backdrop")?.addEventListener("click", () => setAdminOverlay(false));
  logoutBtn?.addEventListener("click", () => {
    localStorage.removeItem("ramsesAdminLogged");
    setAdminOverlay(true);
  });
}

document.addEventListener("DOMContentLoaded", initAdminLoginUi);
